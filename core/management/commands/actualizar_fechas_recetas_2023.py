...
class Command(BaseCommand):
    help = 'Actualiza fecha_prescripcion de recetas y fecha_dispensacion de detalles para que coincidan con la cita asociada (solo recetas de 2023).'

    def handle(self, *args, **options):
        recetas_actualizadas = 0
        detalles_actualizados = 0
        warnings = 0
        recetas = RecetaMedica.objects.filter(cita__isnull=False, cita__fecha__year=2023)
