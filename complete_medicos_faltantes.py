...
def generar_dni(especialidad_id, indice):
    # Formato: 2EEIII (2 + ID especialidad de 2 dígitos + índice de 3 dígitos)
    return f"2{especialidad_id:02d}{indice:03d}"
...
# Verificar y completar médicos faltantes para cada especialidad
for nombre_especialidad, cantidad_requerida in especialidades_requeridas.items():
    try:
        ...
        try:
            ...
            print(f"\nVerificando especialidad: {nombre_especialidad} (ID: {especialidad.id})")
        ...
        print(f"  Médicos existentes: {medicos_existentes}, Requeridos: {cantidad_requerida}")
        
        # Calcular cuántos médicos faltan
        medicos_faltantes = max(0, cantidad_requerida - medicos_existentes)
        
        if medicos_faltantes <= 0:
            print(f"  No se necesitan más médicos para {nombre_especialidad}")
            continue
        
        print(f"  Creando {medicos_faltantes} médicos faltantes para {nombre_especialidad}")
        
        # Crear médicos faltantes en una transacción
        with transaction.atomic():
            creados_especialidad = 0
            
            for i in range(1, medicos_faltantes + 1):
                try:
                    ...
                    print(f"    Creado médico {i}/{medicos_faltantes}: Dr. {nombres} {apellidos} (DNI: {dni}, CMP: {cmp})")
                    
                except Exception as e:
                    print(f"    Error al crear médico {i}/{medicos_faltantes}: {str(e)}")
                    total_errores += 1
